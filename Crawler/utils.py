...
def Get_xpath_group(xpath):
    ...
    try :
        ...
        div_lv2_findall = re.findall('(/.*)(li\[*\d*\]*|ul\[*\d*\]*)(/a|/form|/h2|/h3|/h4|/img|/li|/p|/span|/ul|/strong/)'
                                        ,'/'+ xpath[len(div_lv1)+1:] +'/')
        if div_lv2_findall != []:  
...
def Get_page_xpath(clean_body_tag , status_text:int , print_out = False):
    ...
    '''cant contain below tags\r\n'link','br','p','pre','hr','b','i','u','sup','sub','font','tr','th','td','bgsound','embed','em',
            'map','script','noscript','time','blockquote','area','button','input' '''
...
def Get_attributes(browser , xpath_list , get_data_type_list):
    ...
    print(f'\033[1m{len(duplicate_list)} 筆重覆未寫入資料\033[0m')
